...
* Parameters influencing access cannot be tampered with. 

##### Level 2
* State governing access should not be modifiable, unless authorised.
* All access decisions are logged, including failures.
* Rate limiting is in-place to reasonably limit requests for and modification of access rights by a user.
...
* [SQL injection](https://en.wikipedia.org/wiki/SQL_injection) is prevented.
* [LDAP injection](https://www.owasp.org/index.php/LDAP_injection) is prevented.
* [OS Command injection](https://www.owasp.org/index.php/Command_Injection) is prevented.
...
* [XML attacks](https://en.wikipedia.org/wiki/File_inclusion_vulnerability) are validated against, ensuring document integrity and preventing external entity attacks. 
...
* Data inputted as HTML is santised and escaped.

##### Level 2
* Automatic [mass assignment](https://www.owasp.org/index.php/Mass_Assignment_Cheat_Sheet) appropriately white-lists or black-lists attributes.
...
* Audit logs provide evidence of the occurrence of key transaction (non-repudiation).
...
* Security logs cannot be modified and limit access via authorisation controls
...
##### Level 1
* Sensitive fields are not cached client-side, including autocomplete's.
* Verify sensitive data is not sent clear text via HTTP (e.g. as parameters).
...
* Alerting detects data harvesting is taking place. 

#### Communications
__Purpose:__ Ensure the security of all comms.
...
* [Forward secrecy ciphers](https://en.wikipedia.org/wiki/Forward_secrecy) mitigate recording of traffic.
...
* [TLS certificate public key pinning (HPKP)](https://en.wikipedia.org/wiki/HTTP_Public_Key_Pinning) is in-place with backup keys.
...
##### Level 3
* The application is sandboxed, preventing access to other components and applications.
...
* Data is not used from untrusted parties via [Cross Origin Resource Sharing (CORS)](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS).
* Do not use technologies not natively supported by browsers (such as Flash or Silverlight). 
...
##### Level 1
* Device identifiers are not used for authentication tokens.
* Sensitive data is not stored unencrpyted or unprotected on device.
* Secrets (e.g. API tokens) are dynamically generated for mobile applications.
* Application memory is organised randomly (e.g. via [ASLR](https://en.wikipedia.org/wiki/Address_space_layout_randomization)).
